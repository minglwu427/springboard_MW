...
SELECT CONCAT( sub.firstname, " ", sub.surname ) AS fullname, f.name AS facility,
CASE
WHEN sub.memid =0
THEN f.guestcost
ELSE f.membercost
END AS cost
FROM (
SELECT b.facid, b.memid, surname, firstname, starttime
FROM Bookings AS b
INNER JOIN Members AS m ON m.memid = b.memid
) AS sub
INNER JOIN Facilities AS f ON f.facid = sub.facid
